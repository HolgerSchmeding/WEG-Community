...
export interface Ticket {
  ...
  requester: string;
  date: string;
  status: "Erstellt" | "Offen" | "In Bearbeitung" | "Wartend" | "Zur Erledigung" | "Erledigt";
...
const initialTickets: Ticket[] = [
  ...
  },
  {
    ...
    message: "Die Glühbirne im 3. OG ist seit gestern defekt und sollte ausgetauscht werden.",
  ...
  },
  {
    ...
    reminderDate: null,
    category: "maintenance",
    message: "In der Garage ist ein größerer Wasserfleck an der Decke aufgetreten.",
...
export function TicketProvider({ children }: { children: React.ReactNode }) {
  ...
  const addTicket = React.useCallback((ticketData: Omit<Ticket, 'id' | 'createdAt'>): string => {
    const now = new Date();
  ...
  const updateTicket = React.useCallback((id: string, updates: Partial<Ticket>) => {
    setTickets(prev => 
      prev.map(ticket => 
  ...
  const getTicket = React.useCallback((id: string) => {
    return tickets.find(ticket => ticket.id === id);
  }, [tickets]);

  const getUserTickets = React.useCallback((requester: string) => {
