...
const DocumentSearchInputSchema = z.object({
  ...
  documents: z.array(DocumentSchema).describe('Available documents to search through'),
...
const DocumentSearchOutputSchema = z.object({
  relevantDocuments: z.array(
    z.object({
      ...
      keyExtracts: z.array(z.string()).describe('Key relevant excerpts from the document')
    })
  ).describe('Documents ranked by relevance'),
  searchSummary: z.string().describe('Summary of what was found'),
  suggestedQueries: z.array(z.string()).describe('Alternative search suggestions')
});

export type DocumentSearchOutput = z.infer<typeof DocumentSearchOutputSchema>;

export async function intelligentDocumentSearch(input: DocumentSearchInput): Promise<DocumentSearchOutput> {
