...
export default function RoleTestPage() {
  ...
  const roles: {
  ...
  }[] = [
    {
      ...
      icon: Settings,
      description:
        'Vollzugriff auf alle Bereiche, kann Aushänge erstellen und Benutzer verwalten',
    },
    {
      role: 'hausverwalter',
        ...
        <div className="mb-6">
          <Button asChild className="mb-4" size="lg">
            <Link href={getDefaultPageForRole(currentRole)}>
              ...
              {roles.find(r => r.role === currentRole)?.name})
            </Link>
          </Button>
        </div>
        ...
        <Card className="mb-8">
          ...
          <CardContent>
            <p className="text-muted-foreground mb-4">
              Diese Seite ermöglicht es Ihnen, zwischen verschiedenen
              Benutzerrollen zu wechseln, um die Berechtigungen zu testen.
            </p>
            <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg">
              <div className="text-sm font-medium text-green-900 flex items-center gap-2">
                <Badge variant="default">
                  {roles.find(r => r.role === currentRole)?.name || currentRole}
                </Badge>
                ist aktuell aktiv
              </div>
            </div>
