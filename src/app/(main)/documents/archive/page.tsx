...
import { IntelligentDocumentSearch, type Document as SearchDocument } from "@/components/intelligent-document-search";
...
const mockCurrentBills: CurrentBill[] = [
  ...
  },
  {
    ...
    uploadedBy: "Hausverwaltung Schmidt & Partner", 
    content: "Sonderumlage für die Dachsanierung des Gebäudes. Gesamtkosten der Maßnahme: 135.500€. Ihr Miteigentumsanteil: 2,1%. Ihr Anteil: 2.845,50€. Die Arbeiten umfassten: Vollsanierung des Dachstuhls, neue Dämmung nach EnEV-Standard, Austausch aller Dachziegel, neue Dachrinnen und Fallrohre. Finanzierung teilweise über Instandhaltungsrücklage (1.500€), Rest als Sonderumlage.",
    status: "Neu",
...
export default function DocumentsArchivePage() {
  ...
  const [currentBills, setCurrentBills] = React.useState<CurrentBill[]>(mockCurrentBills);
  const [isViewDialogOpen, setIsViewDialogOpen] = React.useState(false);
  const [isBillViewDialogOpen, setIsBillViewDialogOpen] = React.useState(false);
  const [isSummarizeDialogOpen, setIsSummarizeDialogOpen] = React.useState(false);
  const [selectedDocument, setSelectedDocument] = React.useState<Document | null>(null);
  const [selectedBill, setSelectedBill] = React.useState<CurrentBill | null>(null);
  ...
  const userCurrentBills = React.useMemo(() => {
    ...
    return currentBills.filter(bill => 
      bill.assignedToUserId === `user_${user.name.toLowerCase().replace(' ', '_')}`
    );
  ...
  const searchDocuments: SearchDocument[] = allDocuments.map(doc => {
    ...
    const generateTags = (content: string, category: string, name: string) => {
      ...
      if (contentWords.includes('heizung') || contentWords.includes('energie') || contentWords.includes('wärme')) baseTags.push('Heizung');
      if (contentWords.includes('geld') || contentWords.includes('euro') || contentWords.includes('kosten')) baseTags.push('Finanzen');
      if (contentWords.includes('reparatur') || contentWords.includes('sanierung') || contentWords.includes('instandhaltung')) baseTags.push('Instandhaltung');
      if (contentWords.includes('versammlung') || contentWords.includes('protokoll') || contentWords.includes('sitzung')) baseTags.push('Versammlung');
      if (contentWords.includes('garage') || contentWords.includes('parkplatz') || contentWords.includes('fahrzeug')) baseTags.push('Parken');
      if (contentWords.includes('müll') || contentWords.includes('reinigung') || contentWords.includes('sauberkeit')) baseTags.push('Ordnung');
      if (contentWords.includes('lärm') || contentWords.includes('ruhe') || contentWords.includes('nachbarschaft')) baseTags.push('Nachbarschaft');
      if (contentWords.includes('garten') || contentWords.includes('spielplatz') || contentWords.includes('außenanlage')) baseTags.push('Außenanlagen');
    ...
    return {
      ...
      type: doc.type,
      date: doc.date,
      author: doc.category === 'Protokolle' ? 'Verwaltungsbeirat' : 'Hausverwaltung',
  ...
  const canViewBills = user?.roles?.includes('owner') || user?.roles?.includes('admin') || user?.roles?.includes('hausverwalter');
  
  // Debug: Zeige aktuelle Benutzerrolle in der Konsole
  React.useEffect(() => {
    if (user) {
      console.log('Current user roles:', user.roles, 'canViewBills:', canViewBills);
    }
  }, [user, canViewBills]);
  ...
  const BillTable = ({ bills, onViewBill }: { bills: CurrentBill[], onViewBill: (bill: CurrentBill) => void }) => (
    <>
          ...
          <TableBody>
            {bills.map((bill) => (
              <TableRow key={bill.id}>
                ...
                <TableCell className={`font-medium ${bill.amount > 0 ? 'text-red-600' : 'text-green-600'}`}>
                ...
                <TableCell>
                  <Badge variant={bill.status === 'Neu' ? 'destructive' : 'default'}>
                    {bill.status}
                  </Badge>
                </TableCell>
                <TableCell>
  ...
  return (
    <>
          ...
          {!canViewBills && (
            <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
              <p className="text-sm text-amber-800">
                <strong>Hinweis:</strong> Als {user?.roles?.[0] === 'resident' ? 'Bewohner' : user?.roles?.[0] === 'board' ? 'Beiratsmitglied' : 'Benutzer'} haben Sie keinen Zugriff auf das Abrechnungen-Archiv. 
                Nur Eigentümer und Hausverwaltung können Abrechnungen einsehen. 
                <Link href="/role-test" className="underline ml-2">Rolle für Demo-Zwecke wechseln →</Link>
              </p>
            </div>
          )}
        </div>
        ...
        <div className="max-w-5xl mx-auto mt-12">
            <div className="flex items-center justify-between mb-6 gap-4">
                ...
                >
                  ...
                  {activeTab === "search" ? "Normale Ansicht" : "Intelligente Suche"}
                </Button>
            </div>
            ...
            <Card>
              <CardContent className="p-0">
                 <Tabs defaultValue="protokolle" className="w-full">
                  <TabsList className={`grid w-full ${canViewBills ? 'grid-cols-4' : 'grid-cols-3'}`}>
                    ...
                    {canViewBills && (
                      <TabsTrigger value="abrechnungen" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:shadow-none">Abrechnungen</TabsTrigger>
                    )}
                  </TabsList>
                  <div className="p-6">
                      ...
                      {canViewBills && (
                        <TabsContent value="abrechnungen" className="mt-0">
                          <BillTable bills={filteredBills} onViewBill={handleViewBillClick} />
                        </TabsContent>
                      )}
                  </div>
                </Tabs>
      ...
      <Dialog open={isBillViewDialogOpen} onOpenChange={setIsBillViewDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Abrechnung für {selectedBill?.assignedToUserName} - Fällig bis {selectedBill?.dueDate.toLocaleDateString('de-DE')}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className={`text-lg font-bold ${selectedBill && selectedBill.amount > 0 ? 'text-red-600' : 'text-green-600'}`}>
                  {selectedBill && selectedBill.amount > 0 ? `+${selectedBill.amount.toFixed(2)}€` : `${selectedBill?.amount.toFixed(2)}€`}
                </p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-sm">
                  {selectedBill?.billingPeriod.from.toLocaleDateString('de-DE')} - {selectedBill?.billingPeriod.to.toLocaleDateString('de-DE')}
                </p>
              </div>
              <div>
                <Label className="text-sm font-medium text-muted-foreground">Status</Label>
                <Badge variant={selectedBill?.status === 'Neu' ? 'destructive' : 'default'}>
                  {selectedBill?.status}
                </Badge>
              </div>
            </div>
            <div>
              ...
              <div className="mt-2 p-4 bg-muted rounded-lg">
                <p className="text-sm whitespace-pre-wrap">{selectedBill?.content}</p>
              </div>
            </div>
          </div>
        </DialogContent>
