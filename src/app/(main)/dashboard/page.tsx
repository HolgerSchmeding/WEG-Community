...
const getTypeIcon = (type: string) => {
    switch(type) {
        case "Offiziell": return Building;
        case "Gemeinschaft": return PartyPopper;
        case "Private Info": return User;
        default: return Info;
    }
}
    ...
    <div className="container py-8">
      ...
      <div className="text-center max-w-2xl mx-auto my-8">
        ...
        <p className="mt-4 text-lg text-muted-foreground">
            Hier finden Sie alle wichtigen Informationen und Services für Ihren Alltag in der Wohnanlage Silberbach.
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
        <DashboardCard href="/documents" icon={BookOpen} title="Hausordnung" description="Die wichtigsten Regeln für das Zusammenleben" iconBg="bg-primary" />
        <DashboardCard href="/announcements" icon={Megaphone} title="Aushangbrett" description="Aktuelle Mitteilungen und Ankündigungen" iconBg="bg-cyan-500" />
        <DashboardCard href="/contact" icon={Mail} title="Kontakt & Anfragen" description="Anliegen an die Hausverwaltung senden" iconBg="bg-emerald-500" />
        <DashboardCard href="/marketplace" icon={Store} title="Nachbarschafts-Marktplatz" description="Tauschen, helfen und finden" iconBg="bg-purple-500" />
        <DashboardCard href="/resident-appointments" icon={Calendar} title="Termine & Events" description="Alle Termine und Events im Blick" iconBg="bg-blue-500" />
        <DashboardCard href="/surveys" icon={CheckSquare} title="Umfragen & Abstimmungen" description="An gemeinschaftlichen Entscheidungen teilnehmen" iconBg="bg-orange-500" />
      </div>
      ...
      <Card className="w-full">
        <CardHeader className="flex flex-row items-center justify-between">
            <div>
                 ...
                 <p className="text-sm text-muted-foreground">Wichtige Termine und Aktivitäten in der Wohnanlage</p>
            </div>
                    ...
                    {upcomingAppointments.map((item, index) => {
                       ...
                       return (
                        <li key={index} className="flex items-center gap-4 p-3 rounded-lg bg-muted/50">
                            ...
                            <Badge 
                                variant={item.type === 'Offiziell' ? 'secondary' : 'outline'}
                                className={
                                    item.type === 'Gemeinschaft' ? 'bg-purple-100 text-purple-800' :
                                    item.type === 'Private Info' ? 'bg-amber-100 text-amber-800' : ''
                                }
                            >
                                {item.type}
                            </Badge>
...
function DashboardCard({ href, icon: Icon, title, description, iconBg }: { href: string; icon: React.ElementType; title: string; description: string; iconBg: string; }) {
  return (
    <Link href={href}>
