...
const surveys = [
  {
    ...
    description: "Wir möchten klären, wie der Gemeinschaftsraum im Untergeschoss zukünftig genutzt werden soll. Bitte geben Sie Ihre Präferenz an.",
    ...
    description: "Sollen in der Tiefgarage E-Lade-Stationen installiert werden? Die Kosten würden über eine Sonderumlage finanziert.",
        ...
        <div className="container py-8">
            ...
            <div className="max-w-4xl mx-auto mt-8 text-center">
                ...
                <p className="mt-2 text-muted-foreground max-w-2xl mx-auto">
                    Nehmen Sie an aktuellen Umfragen teil und sehen Sie die Ergebnisse vergangener Abstimmungen ein.
                </p>
            </div>
                ...
                {surveys.map(survey => {
                        ...
                        <Card key={survey.id}>
                            <CardHeader>
                                <div className="flex justify-between items-start">
                                    ...
                                    <Badge variant={survey.status === 'closed' ? 'secondary' : 'outline'} className={survey.status === 'closed' ? 'bg-gray-200 text-gray-800' : 'text-green-600 border-green-600'}>
                                ...
                                <div className="text-xs text-muted-foreground pt-4 flex items-center justify-between">
                                    <span className="flex items-center gap-2">Erstellt von: <strong>{survey.creator}</strong></span>
                                    <span className="flex items-center gap-2"><Clock className="h-4 w-4" /> Endet am: {survey.endDate}</span>
                                </div>
                            </CardHeader>
                            <Separator />
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    ...
                                    <div>
                                        ...
                                        {survey.status === 'active' && !hasVoted ? (
                                            <RadioGroup onValueChange={setSelectedOption}>
                                                <div className="space-y-3">
                                                    {survey.options.map(option => (
                                                        <Label key={option.id} className="flex items-center gap-3 p-3 border rounded-md has-[:checked]:bg-accent has-[:checked]:border-primary cursor-pointer">
                                            ...
                                            <div className="flex flex-col items-center justify-center text-center text-muted-foreground bg-muted/50 p-6 rounded-md h-full">
                                                ...
                                                <p className="font-semibold">{survey.status === 'closed' ? 'Diese Umfrage ist beendet.' : 'Sie haben bereits abgestimmt.'}</p>
                                                <p className="text-xs mt-1">Die Ergebnisse sehen Sie nebenan.</p>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                                ...
                                                return (
                                                    <div key={option.id}>
                                                        <div className="flex justify-between items-center mb-1 text-sm">
                                                            ...
                                                            <span className="text-muted-foreground">{option.votes} Stimme(n) ({percentage.toFixed(0)}%)</span>
                                                        </div>
                                        ...
                                        <div className="mt-6 pt-4 border-t text-center text-sm text-muted-foreground">
                                            <p className="flex items-center justify-center gap-2"><Users className="h-4 w-4" /><strong>{totalVotes}</strong> Personen haben insgesamt teilgenommen.</p>
                                        </div>
                                    </div>
                                </div>
                            </CardContent>
                 ...
                 {surveys.length === 0 && (
                    <div className="text-center py-12 text-muted-foreground border-2 border-dashed rounded-lg">
                        ...
                        <h3 className="font-headline text-xl font-semibold">Keine aktiven Umfragen</h3>
                        <p>Momentan gibt es keine Umfragen, an denen Sie teilnehmen können.</p>
                    </div>
                 )}
            </div>
        </div>
