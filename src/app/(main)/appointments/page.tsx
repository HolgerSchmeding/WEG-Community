...
const initialOwnerAppointments = [
  ...
  },
  {
    ...
    type: 'Beiratssitzung',
    description:
      'Der Verwaltungsbeirat bespricht die anstehenden Themen für das vierte Quartal, inkl. Budgetplanung 2025.',
...
const getTypeDetails = (type: string) => {
  switch (type) {
    ...
    case 'Beiratssitzung':
      return { icon: UserCheck, color: 'bg-purple-500' };
    case 'Sonstiges':
      return { icon: Info, color: 'bg-amber-500' };
    default:
      return { icon: GanttChartSquare, color: 'bg-gray-500' };
  }
};
    ...
    try {
      ...
      if (storedAppointments) {
        ...
        localStorage.setItem(
          'owner_appointments',
          JSON.stringify(initialOwnerAppointments)
        );
      }
    } catch (error) {
    ...
    <div className="container py-8">
      ...
      <div className="max-w-4xl mx-auto mt-8 text-center">
        ...
        <p className="mt-2 text-muted-foreground max-w-2xl mx-auto">
          Eine Übersicht aller wichtigen Termine, die für Sie als
                    ...
                    >
                      <div className="flex flex-col items-center justify-center w-20 text-center bg-muted p-3 rounded-md">
                        ...
                        <span className="text-sm text-muted-foreground">
                          {format(itemDate, 'MMM yyyy', { locale: de })}
                        </span>
                      </div>
                      <div className="flex-grow">
                        <div className="flex justify-between items-start">
                          ...
                          <Badge
                            variant={
                              item.type === 'Versammlung'
                                ? 'secondary'
                                : 'outline'
                            }
                            className={
                              getTypeDetails(item.type)
                                .color.replace('bg-', 'bg-')
                                .replace('500', '500/10') +
                              ' ' +
                              getTypeDetails(item.type).color.replace(
                                'bg-',
                                'text-'
                              )
                            }
