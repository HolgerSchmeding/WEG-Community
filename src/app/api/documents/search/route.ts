...
export async function POST(request: NextRequest) {
  try {
    const { query, mode, documents, userRole }: SearchQuery =
      await request.json();

    if (!query.trim()) {
      return NextResponse.json({
        results: documents,
        summary: 'Alle Dokumente angezeigt.',
      });
    }
      ...
      const results = documents
        ...
        .sort((a: any, b: any) => {
          ...
          const scoreA = expandedTerms.reduce((score, term) => {
            ...
            const tagMatch = a.tags?.some((tag: string) =>
          ...
          const scoreB = expandedTerms.reduce((score, term) => {
            ...
            const tagMatch = b.tags?.some((tag: string) =>
              tag.toLowerCase().includes(term)
            )
              ? 3
              : 0;
