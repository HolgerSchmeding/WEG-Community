...
export async function POST(request: NextRequest) {
  try {
    ...
    const improvedText = await generateProtocolText(keywords, context, discussion);
...
async function generateProtocolText(keywords: string, context: string, discussion: string): Promise<string> {
  ...
  if (keywordLower.includes('instandhaltung') || keywordLower.includes('reparatur')) {
...
Die Eigentümergemeinschaft erörterte die Dringlichkeit und Kostenschätzungen. Nach ausführlicher Beratung wurde der Kostenrahmen und die Ausführungsmodalitäten besprochen.`;
  ...
  if (keywordLower.includes('hausgeld') || keywordLower.includes('nachzahlung') || keywordLower.includes('erhöhung')) {
...
Mehrere Eigentümer äußerten Bedenken bezüglich der Höhe und des Zeitpunkts. Alternative Finanzierungswege wurden diskutiert, jedoch als nicht praktikabel eingestuft.`;
  ...
  if (keywordLower.includes('modernisierung') || keywordLower.includes('sanierung')) {
